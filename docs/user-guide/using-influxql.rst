...
To execute InfluxQL queries you can use the ``send_custom_query()`` method in conjunction with the EFD client.
...
.. code::
   ...
   await client.send_custom_query(query)
...
The EFD client helper function ``build_time_range_query()`` can also be used to build InfluxQL queries for a topic and time range, which can then be executed with the ``send_custom_query()`` method.
