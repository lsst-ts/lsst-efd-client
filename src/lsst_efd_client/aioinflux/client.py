...
class InfluxDBClient:
    def __init__(
        ...
        **kwargs,
        ...
        The three main public methods are the three endpoints of the
        InfluxDB API, namely:
        ...
        :param timeout: Timeout in seconds or :class:`aiohttp.ClientTimeout`
            object
        :param database: Default database to be used by the client.
            This field is for argument consistency with the official
    ...
    async def ping(self) -> dict:
        ...
        Returns a dictionary containing the headers of the response
        from ``influxd``.
        """
    ...
    async def write(
        ...
        **extra_tags,
        ...
        :param measurement: Measurement name. Mandatory when when writing
            DataFrames only. When writing dictionary-like data, this field is
            treated as the default value for points that do not contain a
            `measurement` field.
    ...
    async def query(
        ...
        *,
        ...
        Please refer to the InfluxDB documentation for all possible queries:
        ...
        :param epoch: Precision level of response timestamps.
            Valid values: ``{'ns', 'u', 'µ', 'ms', 's', 'm', 'h'}``.
        :param chunked: If ``True``, makes InfluxDB return results in streamed
            batches rather than as a single response.
            Returns an AsyncGenerator which yields responses
            in the same format as non-chunked queries.
        :param chunk_size: Max number of points for each chunk. By default,
            InfluxDB chunks responses by series or by every 10,000 points,
