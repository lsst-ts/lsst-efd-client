...
def is_optional(t, base_type):
    ...
    # NOTE: The 'typing' module is still "provisional" and documentation
    # sub-optimal, which requires these kinds instrospection into undocumented
...
def lineprotocol(
    ...
    *,
    ...
    :param rm_none: Whether apply a regex to remove ``None`` values.
        If ``False``, passing ``None`` values to boolean, integer or float or
        time fields will result in write errors. Setting to ``True`` is "safer"
        but impacts performance.
    :param extra_tags: Hard coded tags to be added to every point generated.
    :param placeholder: If no field attributes are present, add a placeholder
        attribute (``_``) which is always equal to ``True``.
        This is a workaround for creating field-less points
    ...
    def _lineprotocol(cls):
        ...
        # TODO: Raise warning or exception if schema has optionals
        # but rm_none is False
